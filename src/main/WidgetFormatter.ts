...
export class WidgetFormatter {
    ...
    private static autoDimOrHide(context: ExtensionContext<Host>, command: ParsedCommand<Host>, { dim, hide }: { dim: boolean; hide: boolean; }) {
        if (hide) {
            context.builder.add(command.commandNode.from, command.commandNode.to, Decoration.mark({ attributes: { "class": "derammo-known-tags derammo-known-tags-auto-hide" } }));
        } else if (dim) {
            context.builder.add(command.commandNode.from, command.commandNode.to, Decoration.mark({ attributes: { "class": "derammo-known-tags derammo-known-tags-auto-dim" } }));
        } else {
            context.builder.add(command.commandNode.from, command.commandNode.to, Decoration.mark({ attributes: { "class": "derammo-known-tags" } }));
        }
    }
}
